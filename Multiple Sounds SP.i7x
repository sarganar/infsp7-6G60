Version 1 of Multiple Sounds Sp by Massimo Stella begins here.

"Provee una solucion para reproducir audio en varios canales con loops, sobre Glulx. Traductor: Sarganar."
...
if (glk_gestalt(gestalt_Sound,0) && glk_gestalt(gestalt_SoundMusic,0))  { 
	...
	"Tu interprete no soporta reproduccion de sonidos.";
...
if (glk_gestalt(gestalt_SoundVolume,0)) {
	if ((val <= 5) && (val >=0)) {
		...
		"Nivel de volumen invalido: por favor el nivel esta entre 0 y 5.";
		!"Invalid Volume Level: please insert a value between 0 and 5.";
		}
	}
else {
	"Tu interprete no soporta control de volumen.";
...
if (glk_gestalt(gestalt_Sound,0) && glk_gestalt(gestalt_SoundMusic,0))  { 
	...
	"Tu interprete no soporta detener sonido.";
